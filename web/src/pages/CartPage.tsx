...
export default function CartPage() {
  ...
  const start = async (): Promise<void> => {
    try {
      ...
      if (items.length === 0) throw { code: 'empty_cart' };
      if (!form.name || !form.email) throw { code: 'missing_fields' };
  ...
  const confirmCard = async () => {
    ...
    try {
      ...
      if (r.status === 'succeeded' && r.payment_ref && r.sale_id != null && r.total != null) {
        ...
        setErrorCode(r.code ?? 'payment_failed');
      }
    } catch (e: any) {
      if (e.message === 'Failed to fetch' || e.name === 'TypeError') {
        setErrorCode('network_error');
  ...
  const confirmOtp = async () => {
    ...
    try {
      ...
      if (r.status === 'succeeded' && r.payment_ref && r.sale_id != null && r.total != null) {
        ...
        setErrorCode(r.code ?? 'otp_failed');
      }
    } catch (e: any) {
      if (e.message === 'Failed to fetch' || e.name === 'TypeError') {
        setErrorCode('network_error');
    ...
    <div className="max-w-4xl mx-auto p-4">
      ...
      {items.length === 0 ? (
        ...
        <>
          ...
          {errorCode && <ErrorCard code={errorCode} />}
        </>
      )}
        ...
        {intent && (
          <>
            <div className="text-sm text-gray-600 mb-2">
              Intent: <code>{intent.id}</code> · Total:{' '}
              <b>
            ...
            <div className="text-xs text-gray-600 mt-2">
              Pruebas: 4242 4242 4242 4242 = éxito · 4000 0000 0000 3220 = requiere 3DS · 0002 =
              rechazada · 9995 = fondos insuf. · 0069 = vencida
            </div>
            <button
              onClick={confirmCard}
              disabled={busy}
